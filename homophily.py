...
def node_homophily(A, labels):
    ...
    edge_idx = torch.tensor(
        np.vstack((src_node.cpu(), targ_node.cpu())), dtype=torch.long
    ).contiguous()
    labels = torch.tensor(labels)
    num_nodes = A.shape[0]
    return node_homophily_edge_idx(edge_idx, labels.cpu(), num_nodes)

def node_homophily_abs(A, labels):
    """average of homophily for each node"""
    ...
    labels = torch.tensor(labels)
    num_nodes = A.shape[0]
    return node_homophily_edge_idx_abs(edge_idx, labels.cpu(), num_nodes)
...
def node_homophily_edge_idx_abs(edge_index, labels, num_nodes):
    """edge_idx is 2 x(number edges)"""
    # edge_index = remove_self_loops(edge_idx)[0]
    hs = torch.zeros(num_nodes)
    degs = torch.bincount(edge_index[0, :]).float()
    matches = (labels[edge_index[0, :]] == labels[edge_index[1, :]]).float()
    mismatches = (labels[edge_index[0, :]] != labels[edge_index[1, :]]).float()
    hs1 = hs.scatter_add(0, edge_index[0, :], matches)
    hs2 = hs.scatter_add(0, edge_index[0, :], mismatches)
    h = torch.abs(hs1 - hs2) / degs
    return h[degs != 0].mean()
...
def aggregation_homophily(features, adj, label, modified=True):
    ...
    return torch.mean(torch.argmax(weight_matrix, 1).eq(labels.cpu()).float())